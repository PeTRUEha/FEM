...
class Edge:
    ...
    def addEdgeInfo(node1, node2, element):
...
class Element:
    ...
    def __init__(self, ID, node_ids):
        def calculateArea(self):
            def TriangleArea2D(v1, v2, v3):
                M = matrix([[v1.x - v3.x, v1.y -v3.y],
                      [v2.x - v3.x, v2.y - v3.y]])
                area = abs(linalg.det(M) / 2)
                return area

            def QuadrilateralArea2D(v1, v2, v3, v4):
                area = abs(v1.x * v2.y - v1.y * v2.x +
                    v2.x * v3.y - v2.y * v3.x +
                    v3.x * v4.y - v3.y * v4.x +
                    v4.x * v1.y - v4.y * v1.x) / 2
                return area

            nodes = self.nodes
            if len(nodes) == 3:
                return TriangleArea2D(*nodes)
            elif len(nodes) == 4:
                return QuadrilateralArea2D(*nodes)
        ...
        self.area = calculateArea(self)

        for i in range(-1, len(self.nodes) - 1, 1):
            Edge.addEdgeInfo(self.nodes[i], self.nodes[i + 1], self)
    ...
    def inverseJacobian(self, ksi = 0, eta = 0):
        """ksi = Ax + b, это A"""
        if len(self.nodes) == 3:
            n = self.nodes
    ...
    def getKsiEta(self, x, y):
        A = self.inverseJacobian()
        b = self.Intercept()
        ksiEta = (A * [[x], [y]] + b)
        ksi, eta = float(ksiEta[0][0]), float(ksiEta[1][0])
        return ksi, eta

    def getFormFunc(self, node, x, y):
        (ksi, eta) = self.getKsiEta(x, y)
    ...
    def covers(self, x, y):
        ...
        if (isToTheLeft(p, p1, p2) * isToTheLeft(p3, p1, p2) >= 0
          and isToTheLeft(p, p1, p3) * isToTheLeft(p2, p1, p3) >= 0
          and isToTheLeft(p, p2, p3) * isToTheLeft(p1, p2, p3) >= 0):
            return True
        else:
            return False
    def getValue(self, kind, x, y):
        return sum([self.getFormFunc(node, x, y) * node.values[kind]
          for node in self.nodes])
...
def Visualization():
    ...
    values = [probeLocation('displacement', x, 0) for x in line]
    eps = [probeLocation1('strain', x, 0) for x in line]
    sigma = [probeLocation1('stress', x, 0) for x in line]
    ...
    values = [[probeLocation('displacement', x, y) for x in X]  for y in Y]
    eps = [[probeLocation1('strain', x, y) for x in X] for y in Y]
...
def plot_stiffness(K):
    ...
    array = K.toarray()
    c_y = stiffness.pcolor(range(2 * N), range(2 * N), array, cmap='plasma', vmin=np.nanmin(array), vmax=np.nanmax(array))
    plt.colorbar(c_y)
    plt.show()
